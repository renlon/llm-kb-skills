# kb-arc Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a `/kb-arc` skill that archives substantive Q&A from tutoring sessions into the MLL lessons repository with intelligent merge and auto git-push.

**Architecture:** Single SKILL.md file containing all workflow instructions for Opus single-pass execution. No code files -- the skill is a prompt that guides Opus through conversation scanning, topic grouping, lesson matching, merge/create, README update, and git sync.

**Tech Stack:** Claude Code skill (markdown prompt), git CLI

---

### Task 1: Create the kb-arc SKILL.md

**Files:**
- Create: `plugins/kb/skills/kb-arc/SKILL.md`

- [ ] **Step 1: Create the skill directory**

```bash
mkdir -p plugins/kb/skills/kb-arc
```

- [ ] **Step 2: Write SKILL.md with frontmatter and full workflow**

Create `plugins/kb/skills/kb-arc/SKILL.md` with the following content:
...
```bash
head -5 plugins/kb/skills/kb-arc/SKILL.md
```

Expected: The YAML frontmatter opening with `---`, `user-invocable: true`, etc.

- [ ] **Step 4: Commit**

```bash
git add plugins/kb/skills/kb-arc/SKILL.md
git commit -m "feat: add kb-arc skill for session Q&A archival"
```

### Task 2: Bump plugin version to 1.4.0

**Files:**
- Modify: `plugins/kb/.claude-plugin/plugin.json`
- Modify: `.claude-plugin/marketplace.json`

A pre-commit hook auto-bumps the patch version on every commit. Since this is a new skill (minor feature), we set the version to 1.4.0 explicitly so users can `update` the plugin and get the new skill.

- [ ] **Step 1: Verify skill auto-discovery**

Check that the skill directory follows the same convention as existing skills:

```bash
ls -la plugins/kb/skills/
```

Expected: `kb/`, `kb-init/`, `research/`, `research-add-fields/`, `research-add-items/`, `research-deep/`, `research-report/`, and now `kb-arc/` -- each containing a `SKILL.md`.

- [ ] **Step 2: Set version to 1.4.0 in both plugin JSON files**

Edit `plugins/kb/.claude-plugin/plugin.json`: change the current version to `"1.4.0"`.
Edit `.claude-plugin/marketplace.json`: change the current version to `"1.4.0"`.

The pre-commit hook will bump patch from here on subsequent commits (1.4.0 -> 1.4.1 etc.).

- [ ] **Step 3: Commit**

```bash
git add plugins/kb/.claude-plugin/plugin.json .claude-plugin/marketplace.json
git commit -m "chore: bump plugin version to 1.4.0 for kb-arc skill"
```
...
- [ ] **Step 1: Add kb-arc to the Skills list in CLAUDE.md**

In the `## Skills` section, add a new bullet after the existing skills:
...
```bash
git add CLAUDE.md
git commit -m "docs: add kb-arc to skills list in CLAUDE.md"
```

### Task 4: Manual smoke test

No automated tests -- this is a prompt-only skill. Verify it works by invoking it.

- [ ] **Step 1: Verify skill is discoverable**

In a Claude Code session in the `llm-kb-skills` project directory, check that `/kb-arc` appears in the skill list. The skill description should mention "archive", "session wrap-up", "save lessons".

- [ ] **Step 2: Test invocation in a session with Q&A**

Have a short technical conversation (ask a question, get an answer), then run `/kb-arc`. Verify:
- It scans the conversation and identifies the topic
- It checks existing lessons in `~/Documents/MLL/lessons/`
- It creates or merges a lesson file correctly
- It updates README.md
- It commits and pushes to remote

- [ ] **Step 3: Test merge behavior**

Run a session that discusses a topic already covered by an existing lesson. Run `/kb-arc` and verify:
- The existing lesson is updated (not duplicated)
- The file is renamed with today's date
- The technical summary is merged, not appended
- Only new Q&A is added to the transcript
